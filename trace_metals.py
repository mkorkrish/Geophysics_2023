...
# Correlation Matrix
numeric_vars = ["Dolomite/Ankerite", "Calcite", "Apatite", "Siderite", "Anhydrite", "Gypsum", "Hematite", "Sphalerite", "Pyrite/Maracasite", "Quartz", "Orthoclase", "Plagioclase", "Total Clays", "LECO TOC"]  # Add other numeric variables as needed
...
plt.colorbar()
plt.xticks(range(len(numeric_vars)), numeric_vars, rotation=90)
...
for var in numeric_vars:
    ...
    X = df.dropna(subset=[var])[numeric_vars].copy()  # Independent variables
    y = df.dropna(subset=[var]).loc[X.index, var]  # Dependent variable
    ...
    print(f"Length of model.coef_: {len(model.coef_)}")

    if len(X.columns) != len(model.coef_):
        print("Warning: Length mismatch between X.columns and model.coef_")
        continue
